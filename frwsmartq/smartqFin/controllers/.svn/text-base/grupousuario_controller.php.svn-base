...
class GrupousuarioController extends ApplicationController {
	 ...
	 *
	 */
	public function editarAction($grupo_id=null,$usuario_id=null){

		$filter = new Filter();
		$grupoId = $filter->applyFilter($grupoId,"int");
		$usuarioId = $filter->applyFilter($usuarioId,"int");
		$grupousuario = $this->Grupousuario->findFirst($grupo_id,$usuario_id);
		if($grupousuario){
	 ...
	 *
	 */
	public function guardarAction($isEdit=false,$grupo_id=null,$usuario_id=null){
	...
	public function obtenerDatosGridAction(){
		...
		for($i=$inicio;$i<$limite;$i++){
			...
			$jqgrid->rows[]=array('id'=>$Grupousuario->getGrupoId(),'cell'=>array($Grupo->getNombreLargo(),$Usuario->getCi()));
		}

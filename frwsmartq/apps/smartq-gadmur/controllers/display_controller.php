...
class DisplayController extends ApplicationController {
    ...
    public function initialize() {
        ...
        $plantilla = $sql->getResultSet()->getFirst()->getPlantilla();

        $url = "file:///" . $_SERVER['DOCUMENT_ROOT'] . Core::getInstancePath() . "public/img/$this->carpeta/$plantilla.html";  //frwsmartq
        $plantilla = $this->leer_contenido_completo($url);
        $this->_plantilla = str_replace("sistema", "../../img/$this->carpeta/sistema", $plantilla);
...
//        $result = $db->query("SELECT cajanumero FROM displaycajas ORDER BY id ASC LIMIT 1");
        ...
        //{"t1":"1","v":"1","numcaja":"1","existe":1}
        //$respuesta = array("t1" => "Nelson", "v" => 1, "numcaja" => 1, "existe" => 1);
        return ($respuesta);
    }

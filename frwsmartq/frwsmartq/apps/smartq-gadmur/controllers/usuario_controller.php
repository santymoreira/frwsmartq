...
class UsuarioController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        //usuario.id, nombres, grupo.nombre_largo, grupo.descripcion AS descripcion_grupo, username, estado, ci, telefono, movil, email, foto
        $columnsGrid[] = array('name' => 'Nombres/Apellidos', 'index' => 'nombres');
        $columnsGrid[] = array('name' => 'Grupo de acceso', 'index' => 'nombre_largo', 'width' => '120');
        $columnsGrid[] = array('name' => 'Descripción Grupo', 'index' => 'descripcion_grupo', 'width' => '120');
        $columnsGrid[] = array('name' => 'Username', 'index' => 'username', 'width' => '85');
        $columnsGrid[] = array('name' => 'Estado Usuario', 'index' => 'estado', 'width' => '180');
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        if ($isEdit == FALSE) {
            ...
            if ($verifica_persona) {
                Flash::notice("Ya existe el nombre de usuario " . $verifica_persona->getNombres());
                return;
            }
        }
        ...
        if ($usuario->save() == false) {
            ...
            if ($isEdit == true) {
                ...
                if ($grupo_admin == '0') {  //si ha seleccionado sistema
                    ...
                    $grupo_actual = 11;
                    if ($grupo_dispensador == '4') {
                        $grupo_actual = 4;
                    }
                    if ($grupo_oper_con_ticket == '5') {
                        $grupo_actual = 5;
                    }
                    if ($grupo_pantalla == '6') {
                        $grupo_actual = 6;
                    }
                    if ($grupo_oper_sin_ticket == '7') {
                        $grupo_actual = 7;
                    }
                    $grupo_usuario->deleteAll($whereCondition = "usuario_id= $id");
                    $grupo_usuario->setUsuarioId($id);
                    $grupo_usuario->setGrupoid($grupo_actual);
                    $grupo_usuario->save();
                }
    ...
    public function obtenerDatosGridAction() {
        ...
        $resultado = $db->query("SELECT count(*) as total
            FROM grupousuario
            JOIN usuario ON usuario.id=grupousuario.usuario_id
            JOIN grupo ON grupo.id=grupousuario.grupo_id 
        ...
        $resultado = $db->query("SELECT usuario.id, nombres, grupo.nombre_largo, grupo.descripcion AS descripcion_grupo, username, estado, ci, telefono, movil, email, foto
            FROM grupousuario
            JOIN usuario ON usuario.id=grupousuario.usuario_id
            JOIN grupo ON grupo.id=grupousuario.grupo_id 
            where username <> 'superadmin'
        ...
        while ($row = $db->fetchArray($resultado)) {
            /* $Usuario=$resultado[$i];
              ...
              $jqgrid->rows[]=array('id'=>$Usuario->getId(),'cell'=>array($Usuario->getNombres(),$grupo_acceso,$Usuario->getUsername(),$Usuario->getDescripcion(),$Usuario->getCi(),$Usuario->getTelefono(),$Usuario->getMovil(),"<center>".Tag::image($direccion,'height: 75px')."</center>",$Usuario->getEmail(),$Usuario->getFoto()));
              } */
            $direccion = $this->carpeta . "/fotos/" . $row['foto'];
            $foto = "<center>" . Tag::image($direccion, 'height: 75px') . "</center>";
            $jqgrid->rows[] = array('id' => $row['id'], 'cell' => array($row['nombres'], $row['nombre_largo'], $row['descripcion_grupo'], $row['username'], $row['estado'], $row['ci'], $row['telefono'], $row['movil'], $row['email'], $foto));
        }

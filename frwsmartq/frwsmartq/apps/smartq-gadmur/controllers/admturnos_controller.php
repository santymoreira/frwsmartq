...
class AdmturnosController extends ApplicationController {
    ...
    */
    ...
    public function listaOperadoresTodos($ubicacion_id) {
        ...
        //$modulo_id= $this->getPostParam("modulo_id");
        //$caja= new Caja();
        //$buscaCaja= $caja->find("ubicacion_id1=$ubicacion_id","columns: id, numero_caja");

        $condition = "ubicacion_id=$ubicacion_id";
        $sql = new ActiveRecordJoin(array(
                        "entities" => array("Caja", "Usercaja", "Usuario", "Ubicacion"),
                        "fields" => array(
                                "{#Caja}.id",
                                "{#Caja}.numero_caja",
                                "{#Caja}.descripcion",
        ...
        $html.="<table border='1' cellspacing='0' cellpadding='0'>
            ...
            <th class='td_turno'>Usuario</th> </tr>";
        ...
        foreach ($sql->getResultSet() as $result) {
            ...
            $html.="<td class='td_turno'>{$result->getDescripcion()}</td>";
            $html.="<td class='td_turno'>{$result->getNombres()}</td>";

...
class EmpresaController extends ApplicationController {
    ...
    public function guardarAction($isEdit=false,$id=null) {
        ...
        if($moduloOperadores==1){
            $permiso->updateAll("permiso=1","menu_id IN (30)");
        } else {
            $permiso->updateAll("permiso=0","menu_id IN (30)");
        }
        if($moduloCajas==1){
            $permiso->updateAll("permiso=1","menu_id IN (33)");
        } else {
            $permiso->updateAll("permiso=0","menu_id IN (33)");
        }

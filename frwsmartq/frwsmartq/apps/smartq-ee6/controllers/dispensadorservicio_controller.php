...
class DispensadorservicioController extends ApplicationController {
    ...
    public function initialize() {
        ...
        $sql=new ActiveRecordJoin(array("entities"=>array ("Usuario","Dispensador","Userdispensador"),
                        "fields"=>array (
                                "{#Dispensador}.id",
                                "{#Dispensador}.descripcion",
                                "{#Dispensador}.tipo_dispensador"
                        ),
                        "conditions"=>"{#Userdispensador}.usuario_id=$idsesion"));
    ...
    public function indexAction() {
        $this->routeTo('action: dispensar');
    }

    /**
     * Crear un Dispensadorservicio/
     *
     */
     ...
     * Esta funci�n es llamada cuando se loguea como dispensador
    */
    ...
    */
    public function emitirturnoAction() {
        ...
        if ($prioridad==1)
            $buscaMaxturno=$turno->maximum("numero_alias","conditions: servicio_id= $serv and fecha_emision= '$fech' AND prioridad= 1");
        else
            $buscaMaxturno=$turno->maximum("numero","conditions: servicio_id= $serv and fecha_emision= '$fech'");
        if ($buscaMaxturno) {
            $maxturno=$buscaMaxturno;
        }
        //echo "prioridad".$prioridad; die();
        ...
        //INICIO IMPRIMIR TURNOS
		//____________________
		$nombre_impresora = '';
		$db = DbBase::rawConnect();
		$result2 = $db->query("SELECT 	nombre_impresora	FROM configuracionsistema ;");
        ...
        //$handle = printer_open("PRP-080");
        $handle = printer_open($nombre_impresora);
        //$handle = printer_open("Adobe PDF");
        printer_start_doc($handle, "My Document");
        printer_start_page($handle);

        $y=0;
        if($this->chk_l==1) {
            printer_draw_bmp($handle, "C:/logo_ticket.bmp", 160, 0, 200, 100); //X,Y,W,H
            $y=$y+100;
        }
        if($this->chk_fi==1) {
            $font = printer_create_font("Arial", 20, 15, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font);
            printer_draw_text($handle, $this->fraseinicial, 190, $y);
            $y=$y+15;
        }
        if($this->chk_e==1) {
            $font2 = printer_create_font("Arial", 25, 25, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font2);
            printer_draw_text($handle, $this->empresa, 150, $y);
            $y=$y+50;
        }
        ...
        $logo_ticket = '';
		$db = DbBase::rawConnect();
		$result2 = $db->query("SELECT 	logo_ticket	FROM configuracionsistema ;");
		while($row = $db->fetchArray($result2)) 
		{
        ...
        //FIN IMPRIMIR TURNO
        $datos= array("esperando"=>$total_turnos_esperando,"atendidos"=>$total_turnos_atendidos,"tiempo"=>$tiempo_espera);
        return ($datos);
    }

    /*
     * Con php_printer
     * Para emitir turno por mas de una vez
    */
    public function emitirTurnoAntiguoAction() {
        $this->setResponse("json");
        $operador_id        =$this->getPostParam('operador_id'); //id del servicio
        $serv               =$this->getPostParam('servicio_id'); //id del servicio
        $id_gruposervicio   =$this->getPostParam('gruposervicio_id');
        $letra              =$this->getPostParam('letra');
        $nombre_servicio    =$this->getPostParam('nombre_servicio');
        $nombre_ubicacion   =$this->getPostParam('nombre_ubicacion');
        $numero_modulo      =$this->getPostParam('numero_modulo');
        ...
        $fecha_hoy=date("Y-m-d");
        $db = DbBase::rawConnect();
        $result2 = $db->query("SELECT por_atender, atendido, duracion AS total FROM turnos t, servicio s, gruposervicio gs WHERE gs.id=s.gruposervicio_id AND s.id=t.servicio_id AND gs.id=$id_gruposervicio AND fecha_emision='$fecha_hoy' AND (caja_id=$operador_id OR caja_id IS NULL)");
        while($row2 = $db->fetchArray($result2)) {
            if($row2['por_atender']==0)
                $total_turnos_esperando+=1;
            if ($row2['atendido']==1)
        ...
        $y=0;
        if($chk_l==1) {
            printer_draw_bmp($handle, "C:/$logoticket", 160, 0, 200, 100); //X,Y,W,H
            $y=$y+100;
        }
        if($chk_fi==1) {
            $font = printer_create_font("Arial", 20, 15, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font);
            printer_draw_text($handle, $fraseinicial, 160, $y);
            $y=$y+15;
        }
        if($chk_e==1) {
            $font2 = printer_create_font("Arial", 25, 25, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font2);
            printer_draw_text($handle, $empresa, 150, $y);
            $y=$y+50;
        }
        ...
        if($chk_s==1) {
            $font = printer_create_font("Arial", 20, 12, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font);
            printer_draw_text($handle, $palabra, 160, $y);
            $y=$y+20;

            $font = printer_create_font("Arial", 20, 12, 400, false, false, false, 0); //alto y ancho
            printer_select_font($handle, $font);
            printer_draw_text($handle, "=> ".$nombre_ubicacion." MODULO ".$numero_modulo, 100, $y);
            $y=$y+20;
        }
        ...
        printer_select_font($handle, $font);
        printer_draw_text($handle, "(593-2) 242-5428", 310, $y+14); //x,y*/
        printer_draw_bmp($handle, "C:/telefono_peopleweb.bmp", 481, 25, 35, 225); //X,Y,W,H
...
Tiempo de espera: $tiempo_espera
     ...
     * Para emitir turno por mas de una vez
    */
    public function emitirTurnoAntiguo1Action() {
        $this->setResponse("json");
        $operador_id        =$this->getPostParam('operador_id'); //id del servicio
        $serv               =$this->getPostParam('servicio_id'); //id del servicio
        $id_gruposervicio   =$this->getPostParam('gruposervicio_id');
...
<center>$nombre_ubicacion - MODULO $numero_modulo </center>
<center> $fecha </center>
Clientes en espera: $total_turnos_esperando<br />
Tiempo de espera: $tiempo_espera
</div>
    ...
    /**
     Funci�n que permite recargar la hora en el dispensador
     */
    ...
    /*
     * Funcion que permite mostrar la lista de operadores que est�n el linea y con pausas
    */

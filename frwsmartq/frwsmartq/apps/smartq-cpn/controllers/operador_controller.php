...
class OperadorController extends ApplicationController {
    ...
    public function initialize() {
        ...
        $buscaCajaPausas = $cajaPausas->findFirst("usuario_id=$idsesion AND estado=1");
        if ($buscaCajaPausas)
    ...
    public function siguienteAction() {
        ...
        //-- pregunto si existe uno especial
        $result = $db->query("SELECT t.id, tiempo_maximo, numero, letra, ubicacion_id, hora_emision, letra_alias, numero_alias, prioridad FROM turnos t, servicio s WHERE s.id=t.servicio_id AND  t.estado=0 AND por_atender=0 AND atendido=0 AND rechazado=0 AND (caja_id=$idcaja OR caja_id IS NULL) AND fecha_emision= '$fecha_hoy' AND prioridad=1 AND servicio_id IN (SELECT servicio_id FROM serviciocaja WHERE caja_id=$idcaja AND llamar=1) ORDER BY t.id LIMIT 1");

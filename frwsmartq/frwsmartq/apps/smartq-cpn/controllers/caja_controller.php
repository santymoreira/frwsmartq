...
class CajaController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $columnsGrid[] = array('name' => 'M贸dulo', 'index' => 'numero_caja', 'width' => '50px');
        $columnsGrid[] = array('name' => 'IP', 'index' => 'ip', 'width' => '80px');
        $columnsGrid[] = array('name' => 'Descripci贸n', 'index' => 'descripcion');
        ...
        $columnsGrid[] = array('name' => 'Tipo Calificaci贸n', 'index' => 'tipo_calificacion');
        ...
        $columnsGrid[] = array('name' => 'Ubicaci贸n', 'index' => 'ubicacion_id', 'width' => '80px');
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        if (empty($horario_id))
            ...
            if ($buscaCaja) {
                //$caja->save()=false;
                Flash::error('IP asignada asignado previamente.');
            } else {
                if ($caja->save() == false) {
                    Flash::error('Hubo un error guardando el registro.');
                } else {
                    //INICIO USERCAJA
                    //Actualizar el usuario del usercaja
                    /* if ($isEdit==true) {
                      $usuario_caja= new Usercaja();
                      $usuario_caja->updateAll("usuario_id = $usuario","caja_id= $id");
                      } else { //Edit==false
                      //Actualizamos el usuario_id y caja_id de usercaja
                      $buscaMax= $caja->maximum("id");
                      $usuario_caja = new Usercaja();
                      $usuario_caja->setUsuarioId($usuario);
                      $usuario_caja->setCajaId($buscaMax);
                      $usuario_caja->save();
                      } */
                    //FIN USERCAJA
    ...
    public function obtenerDatosGridAction() {
        ...
        $limite = $this->getPostParam('rows'); // obtener el n锟絤ero de filas que queremos tener en el grid
        $col_orden = $this->getPostParam('sidx'); // Obtener la fila de 锟絥dice - es decir, cuando el usuario haga clic en la columna para ordenar
        $dir_orden = $this->getPostParam('sord'); // obtener la direcci锟絥 de ordenado
        if (!$col_orden)
            $col_orden = 1;
        ...
        for ($i = $inicio; $i < $limite; $i++) {
            ...
            //if($Caja->getEstado()==0)
            //$estado='Inactivo';
            //$jqgrid->rows[]=array('id'=>$Caja->getId(),'cell'=>array($Caja->getNumeroCaja(),$Caja->getDescripcion(),$estado));
            //INICIO BUSCAR USUARIO Y TIPO OPERADOR
            //SELECT nombre_largo FROM caja c, usercaja uc, usuario u, grupousuario gu, grupo g
            //WHERE c.id=uc.caja_id AND u.id=uc.usuario_id AND g.id=gu.grupo_id AND u.id=gu.usuario_id AND c.id=1
            $caja_id = $Caja->getId();
            $condicion = "{#Caja}.id = $caja_id";
            $query = new ActiveRecordJoin(array(
                        "entities" => array("Caja", "Usercaja", "Usuario", "Grupousuario", "Grupo"),
                        "fields" => array(
                            "{#Usuario}.nombres",
                            "{#Grupo}.nombre_largo"),
            ...
            foreach ($query->getResultSet() as $result) {
                $tipo_operador = $result->getNombreLargo();

                $nombre_usuario = $result->getNombres();

                $nombre_usuario = ($nombre_usuario == '' ? 'sasa' : $nombre_usuario);
            }
            //FIN BUSCAR USUARIO Y TIPO OPERADOR
            //INICIO BUSCA TIPO CALIFICAICON
            $tipo_calificacion = "";
            $tipo_calificacion = $Caja->getTipoCalificacionOperador();
            if (strlen($tipo_calificacion) > 1) {
                $tipo_calificacion = substr($tipo_calificacion, 0, 1);
            }
            ...
            if ($Caja->getTransferirUno() == 0)
                $transferir_uno = '<font color="red"><b>No</b></font>';
            if ($Caja->getTransferirTodos() == 0)
                $transferir_todos = '<font color="red"><b>No</b></font>';
            //FIN BUSCA HORARIO

            $Horario = $Caja->getHorario();
            $Ubicacion = $Caja->getUbicacion();
            $jqgrid->rows[] = array('id' => $Caja->getId(), 'cell' => array($Caja->getNumeroCaja(), $Caja->getIp(), $Caja->getDescripcion(), $tipo_calificacion, $Horario->getNombreHorario(), $transferir_uno, $transferir_todos, $Ubicacion->getNombreUbicacion(), $Caja->getTiempo()));
            //$jqgrid->rows[]=array('id'=>$Caja->getId(),'cell'=>array($Caja->getNumeroCaja(),$Caja->getIp(),$Caja->getDescripcion(),$nombre_usuario,$tipo_operador,$tipo_calificacion,$Horario->getNombreHorario(),$transferir_uno,$transferir_todos,$Ubicacion->getNombreUbicacion()));
            //$jqgrid->rows[]=array('id'=>$Caja->getId(),'cell'=>array($Caja->getNumeroCaja(),$Caja->getDescripcion(),$nombre_usuario,$tipo_operador,$tipo_calificacion,$horario,$transferir_uno,$transferir_todos,$nombreUbicacion));
        }

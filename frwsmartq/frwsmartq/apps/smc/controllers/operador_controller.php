<?php
class OperadorController extends ApplicationController {
    ...
    public function initialize() {
        ...
        //SELECT tipo_calificacion_operador FROM usuario u, usercaja uc, caja c
        //WHERE u.id=uc.usuario_id AND c.id=uc.caja_id AND u.id=2
        $sql=new ActiveRecordJoin(array(
                        ...
                        "conditions"=>"{#Usuario}.id=$idsesion"));
        foreach ($sql->getResultSet() as $result) {
            $this->caja_id=$result->getId();
            $this->tipo_calificacion=$result->getTipoCalificacionOperador();
            Tag::displayTo('tipo_calificacion', $result->getTipoCalificacionOperador());
            Tag::displayTo('txt_transferir_uno', $result->getTransferirUno());
            Tag::displayTo('txt_transferir_todos', $result->getTransferirTodos());
        }
        $cajaPausas= new CajaPausas();
        $buscaCajaPausas= $cajaPausas->findFirst("caja_id=$this->caja_id AND estado=1");
        if ($buscaCajaPausas)
    ...
    public function siguienteAction() {
        ...
        $hora_emision="";
        $tiempo_maximo="";
        //selecciona el primer registro de turno segÃºn la caja que llama y la fecha actual
        $db = DbBase::rawConnect();
        //$result = $db->query("SELECT id, servicio_id, numero FROM turnos WHERE estado=0 AND por_atender=0 AND atendido=0 AND rechazado=0 AND fecha_emision= '$fecha_hoy' AND servicio_id IN (SELECT servicio_id FROM serviciocaja WHERE caja_id=$idcaja) LIMIT 1");
        $result = $db->query("SELECT t.id, tiempo_maximo, numero, letra, ubicacion_id, hora_emision FROM turnos t, servicio s WHERE s.id=t.servicio_id AND  t.estado=0 AND por_atender=0 AND atendido=0 AND rechazado=0 AND (caja_id=$idcaja OR caja_id IS NULL) AND fecha_emision= '$fecha_hoy' AND servicio_id IN (SELECT servicio_id FROM serviciocaja WHERE caja_id=$idcaja AND llamar=1) ORDER BY t.id LIMIT 1");
        while($row = $db->fetchArray($result)) {
            $id_turno= $row['id'];
            //$tiempo_maximo= $row['servicio_id'];
        ...
        //FIN BUSCAR PRIMER REGISTRO DE TURNO
        ...
        //INICIO ACTUALIZAR EL TURNO A SER ATENDIDO
        //Actualiza el turno a llamar poniendo en el campo por_atender = 1 para q otro cajero no llame al mismo
        $turno= new Turnos();
        $turno->updateAll("por_atender= 1, caja_id= $idcaja","id= $this->id_turno");
        //FIN ACTUALIZAR EL TURNO A SER ATENDIDO

        //INICIO ENVIAR A DISPLAYTURNO
        //Sirve para ver en la pantalla los turnos
        //INICIO AUMENTAR CEROS EN TURNO
        $turno_displayturno= $actual;
        ...
        $displayturno->setNumeroturno($letra.$turno);
        $displayturno->setFecha($fecha_hoy);
        $displayturno->setUbicacion($ubicacion_id);
        $displayturno->save();
        //FIN ENVIAR A DISPLAYTURNO

        list($espera1,$espera2)=$this->contarTurnosEnEspera();  //ver los turnos en espera


        //$this->activar_siguiente=0; //srve para cuando estï¿½ activo la calificaciï¿½n

        $datosturno= array("actual1"=>$actual,"servicio"=>$letra,"espera1"=>$espera1,"espera2"=>$espera2,
                "hora_emision"=>$hora_emision,"tiempo_maximo"=>$tiempo_maximo,
                "tipo_d"=>$tipo_d,"anulado"=>$this->anulado,"terminado"=>$this->terminado);
        return ($datosturno);
    }
    ...
    public function contarTurnosAction() {
        ...
        $datos= array("espera1"=>$espera1,"espera2"=>$espera2);
        return ($datos);
    }
    ...
    */
     ...
     * parï¿½metros que recibe para turnos_transferidos:
    ...
    */
    public function anularAction() {
        ...
        if ($this->normal==1) {
            $turno= new Turnos();
            $turno->updateAll("rechazado= 1, atendido=1 ","id= $this->id_turno");
        } else {
    ...
    */
    ...
    /**
     *Funcion que permite terminar el turno y enviar la calificaciï¿½n
    ...
    public function listaOperadores1Action() {
        ...
        foreach ($sql->getResultSet() as $result) {
            ...
            if ($caja_id==$this->caja_id)
                $html.="<input type='checkbox' id='chk_modulos_turno_actual_$caja_id' name='chk_modulos_turno_actual_$caja_id' value='$caja_id' onclick='seleccionar_turno_actual()' disabled='disabled' />Módulo $numero_caja<br>";
            else
                $html.="<input type='checkbox' id='chk_modulos_turno_actual_$caja_id' name='chk_modulos_turno_actual_$caja_id' value='$caja_id' class='c_modulos_turno_actual' onclick='seleccionar_turno_actual()' />Módulo $numero_caja<br>";
    ...
    */
    public function listaOperadoresAction() {
        ...
        $query = new ActiveRecordJoin(array(
                        "entities" => array("Caja", "Usuario", "Usercaja","Ubicacion","Servicio","Serviciocaja"),
                        "fields" => array(
                                "{#Caja}.id",
                                "{#Caja}.numero_caja",
                                "{#Usuario}.nombres",
    ...
    /*
     * Funciï¿½n primero las ubicaciones de los Usuario para luego mostrar los modulos
    */
    ...
    */
    public function listaOperadoresTodos1($ubicacion_id) {
        ...
        foreach ($buscaCaja as $result) {
            ...
            $numero_caja=$result->getNumeroCaja();
            if ($caja_id==$this->caja_id)
                $html.="<input type='checkbox' id='chk_modulos_todos_$caja_id' name='chk_modulos_todos_$caja_id' value='$caja_id' onclick='seleccionar($ubicacion_id)' disabled='disabled' />Módulo $numero_caja<br>";
            else
                $html.="<input type='checkbox' id='chk_modulos_todos_$caja_id' name='chk_modulos_todos_$caja_id' value='$caja_id' class='c_modulos_$ubicacion_id' onclick='seleccionar($ubicacion_id)' />Módulo $numero_caja<br>";
        }
    ...
    */
    public function listaOperadoresTodos($ubicacion_id) {
        ...
        $caja= new Caja();
        $buscaCaja= $caja->find("ubicacion_id=$ubicacion_id","columns: id, numero_caja");

        $html.="<fieldset class='ui-corner-all ui-widget-content'>
                    <legend></legend>";

        $array_lista_operadores_aux=array();
        foreach ($buscaCaja as $result) {
            $caja_id=$result->getId();
            $numero_caja=$result->getNumeroCaja();
            if ($caja_id==$this->caja_id){
                //$html.="<input type='checkbox' id='chk_modulos_todos_$caja_id' name='chk_modulos_todos_$caja_id' value='$caja_id' onclick='seleccionar($ubicacion_id)' disabled='disabled' />Módulo $numero_caja<br>";
                $dato="<input type='checkbox' id='chk_modulos_todos_$caja_id' name='chk_modulos_todos_$caja_id' value='$caja_id' onclick='seleccionar($ubicacion_id)' disabled='disabled' />Módulo $numero_caja<br>";
            }else{
                //$html.="<input type='checkbox' id='chk_modulos_todos_$caja_id' name='chk_modulos_todos_$caja_id' value='$caja_id' class='c_modulos_$ubicacion_id' onclick='seleccionar($ubicacion_id)' />Módulo $numero_caja<br>";
    ...
    /*
     * Funcion que hace la transferencia de turno
    */
    ...
    */
    ...
    /*
     * Funcion que permite listar los turnos transferidos
    */
        ...
        foreach ($buscaTurnosTransferidos as $result) {
            ...
            foreach ($separa as $id) {
                if( $this->caja_id == $id) {
                    ...
                    $id=$result->getId();
                    $letra=$result->getLetra();
                    $numero=$fun->aumentaCerosTurno($result->getNumero());
                    ...
                    $html.="<tr> <td>$turno</td><td>Módulo $remitente</td><td>$tiempo_esperando</td><td><input type='button' value='llamar' onclick=". " \"llamar_t_transferido($id,'$letra',$numero,$remitente,$ubicacion_id)\" "."/></td> </tr>";
                }
            }
        }
        $html.="</table>";
    ...
    /*
     * Funciï¿½n que permite llamar al turno actualizando la tabla turnos_transferidos
    ...
    */
    ...
    public function llamarTurnoTransferidoAction() {
        ...
        $turnosTransferidos->updateAll("por_atender= 1, caja_id= $this->caja_id","id= $this->id_t_t_atender");

        //INICIO ENVIAR A DISPLAYTURNO
        //Sirve para ver en la pantalla el turno_transferido
        //INICIO AUMENTAR CEROS EN TURNO
        $turno_displayturno= $turno;
        $contturno=strlen($turno_displayturno);
        if ($contturno == 1)
        ...
        $displayturno->setNumeroturno($servicio.$turno);
        $displayturno->setFecha($fecha_hoy);
        $displayturno->setUbicacion($this->ubicacion_t_t);
        $displayturno->save();
        //FIN ENVIAR A DISPLAYTURNO
        ...
        //$datos= array("letra"=>$letra,"numero"=>$numero,"ubicacion_id"=>$ubicacion_id);
        //return ($datos);
    }
    ...
    */
    ...
    /*
     * Funcion que permite listar los turnos dependiendo del servicio que antes seleccionï¿½
    */

...
class CajeroController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $this->ubicacion_id = $buscaCaja->getUbicacionId();
        ...
        $caja = new Caja();
        $condicion = "ip = '$ip'";
        $caja->updateAll("usuario_actual=$idsesion", "conditions: $condicion");
...
//        $objColas ->setDuracion($duracion);
    ...
    /**
     * Funcion que permite guardar el turno para los cajeros y no los operadores
     */
    ...
    public function siguientecajaAction() {
        ...
        //INICIO SIN CALIFICACION
        //echo "CALIFICACION".$this->tipo_calificacion; die();
        if ($this->tipo_calificacion == "D") {    //A=Teclado, B=P. Simple, C=P. Matriz, D=Ninguno
            //Estaba comentado. El problema q tiene es que no guarda los datos cuando es sin calificacion
            //INICIO PRIMERO ACTUALIZA LA COLA ATENDIDA
            $duracion = "SEC_TO_TIME(TIME_TO_SEC( NOW() ) - TIME_TO_SEC(CONCAT(fecha_inicio_atencion, ' ', hora_inicio_atencion)))";
            $colas = new Colas();
            $colas->updateAll("atendido=1, fecha_fin_atencion='$fecha_fin_atencion', hora_fin_atencion='$hora_fin_atencion', duracion=$duracion, calificacion='NO CALIFICADO'", "caja_id=$idcaja AND atendido=0 AND fecha_inicio_atencion= '$fecha_hoy'");
        ...
        //FIN PARA CALIFICACION CON TECLA
    }

...
class PreguntasController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $columnsGrid[] = array('name' => 'Grupo', 'index' => 'id_grupopregunta');
    ...
    public function editarAction($id = null) {
        ...
        if ($preguntas) {
            ...
            Tag::displayTo('id_grupopregunta', $preguntas->getIdGrupopregunta());
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        $idGrupopregunta = $this->getPostParam("id_grupopregunta", "int");
        ...
        $preguntas->setId($id);
        $preguntas->setIdGrupopregunta($idGrupopregunta);
    ...
    public function obtenerDatosGridAction() {
        ...
        if ($buscar == 'true') { //verificamos si la busqueda es activada
            if ($strbusqueda != '') {    // construccion de la cadena de condicion para la busqueda normal
                switch ($campoBusqueda) {
                    //añadido por yo
                    case 'id_grupopregunta':
                        $condGrupopregunta = Utils::toSqlParamSearchGrid('nom_grupo', $abrevoper, $strbusqueda);
                        $Grupopregunta = $this->Grupopregunta->find($condGrupopregunta);
                        if (count($Grupopregunta) > 0) {
                            $arrayIdsGrupopregunta = array();
                            foreach ($Grupopregunta as $fila) {
                                $arrayIdsGrupopregunta[] = $fila->getId();
                            }
                            $strbusqueda = join(',', $arrayIdsGrupopregunta);
                            $abrevoper = 'in';
                        }
                        break;
                    //fin de todo
            ...
            foreach ($sarr as $k => $v) {
                switch ($k) {
                    case 'id_grupopregunta':
                        $condGrupopregunta = Utils::toSqlParamSearchGrid('nom_grupo', 'bw', $v);
                        $Grupopregunta = $this->Grupopregunta->find($condGrupopregunta);
                        if (count($Grupopregunta) > 0) {
                            $arrayIdsGrupopregunta = array();
                            foreach ($Grupopregunta as $fila) {
        ...
        for ($i = $inicio; $i < $limite; $i++) {
            ...
            $Grupopregunta = $Preguntas->getGrupopregunta();
            $jqgrid->rows[] = array('id' => $Preguntas->getId(), 'cell' => array($Grupopregunta->getNomGrupo(), $Preguntas->getNomPregunta(), "<center>" . $publicar . "</center>", "<center>" . $Preguntas->getOrden() . "</center>"));
        }

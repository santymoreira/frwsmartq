...
class DisplayController extends ApplicationController {
    ...
    */
    public function vercajaAction() {
        ...
        $result = $db->query("SELECT cajanumero FROM displaycajas ORDER BY id DESC LIMIT 1");
        ...
        $respuesta= array("t1"=>$mensajeturno1,"v"=>$timbre,"numcaja"=>$ver);
        return ($respuesta);
    }
    ...
    */
    public function timbrarTurnoAction() {
        ...
        $respuesta= array("reproducir"=>$val);
        return ($respuesta);
    }
    ...
    */
    ...
    /*
     * Permite las noticias RSS segun la pantalla
     * No se usa
    */
    public function verNoticiasRss1Action() {
        include 'lastRSS.php';
        ...
        $html.="<marquee direction='$marquee_direccion' align='$marquee_alineacion' scrollamount='4' width='$marquee_ancho' height='$marquee_alto' id='Marquesina' bgcolor='transparent'>
        <font style='color:#$this->colorNoticias; font-size:$marquee_font_size; font-weight: bold';>.";
    ...
    public function verNoticiasRssAction() {
        ...
        if (!empty($query)) {
            foreach ($query->getResultSet() as $result) {
                $url    =$result->getUrlFeed();
                $limite =$result->getLimiteItems();
                $resultado_rss= RSS_Display_Pantalla($url, $limite, $this->carpeta);
                $array_noticias_aux=explode(',',$resultado_rss);
                foreach($array_noticias_aux as $val){
                    $array_noticias[]=$val;
                }
            }
        }

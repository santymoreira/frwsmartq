...
abstract class Generator {
	...
	static function dumpFieldInformation($form){
		...
		if(!isset($_SESSION['KSF'][$instanceName][$activeApp])){
			$_SESSION['KSF'][$instanceName][$activeApp] = array();
		}
		if(!isset($_SESSION['KSF'][$instanceName][$activeApp][$form['source'].$form['type']])){
			$_SESSION['KSF'][$instanceName][$activeApp][$form['source'].$form['type']] = null;
		}
		if($_SESSION['KSF'][$instanceName][$activeApp][$form['source'].$form['type']]&&!$form['force']){
			$mdata = $_SESSION['KSF'][$instanceName][$activeApp][$form['source'].$form['type']];
			if($mdata['status']=='C'){
				$data = unserialize(gzuncompress($mdata['data']));
			} else {
				$data = unserialize($mdata['data']);
			}
			$form = $data;
			return true;
		}
	...
	static function buildForm($form, $scaffold=false){
		...
		if(Core::fileExists("public/css/$controller_name.css")){
			self::formsPrint("<link rel='stylesheet' href='".Core::getInstancePath()."css/$controller_name.css' type='text/css'/>\n");
		}

		self::formsPrint("<div class='$controller_name'>
		...
		self::formsPrint("<input type='hidden' name='aaction' value='".$controller_name."' />\r\n");

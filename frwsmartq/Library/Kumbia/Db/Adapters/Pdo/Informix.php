 ...
 * @version 	$Id: Informix.php 36 2009-05-05 02:11:38Z gutierrezandresfelipe $
 */
...
class DbPdoInformix extends DbPDO  {
	...
	public function listTables(){
		$query = "SELECT tabname FROM systables WHERE tabtype = 'T' AND version <> 65537";
	...
	public function setIsolationLevel($isolationLevel){
		switch($isolationLevel){
			case self::ISOLATION_READ_UNCOMMITED:
				$this->exec('SET ISOLATION TO DIRTY READ');
				break;
			case self::ISOLATION_READ_COMMITED:
				$this->exec('SET ISOLATION TO COMMITED READ');
				break;
			case self::ISOLATION_REPEATABLE_READ:
				$this->exec('SET ISOLATION TO REPETEABLE READ');
				break;
			case self::ISOLATION_SERIALIZABLE:
				$this->exec('SET ISOLATION TO CURSOR STABILITY');
				break;
		}
	}

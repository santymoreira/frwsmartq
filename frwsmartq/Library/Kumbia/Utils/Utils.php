 ...
 * @version 	$Id: Utils.php 31 2009-05-02 21:59:07Z gutierrezandresfelipe $
 */
...
abstract class Utils {
	...
	public static function ucwords($words){
        ...
        public static function getOperatorLogicGrid($oper){
            $arrayOpe['eq']="=";
            $arrayOpe['ne']="<>";
            $arrayOpe['lt']="<";
            $arrayOpe['le']="<=";
            ...
            $arrayOpe['ge']=">=";
            $arrayOpe['bw']="LIKE";
            $arrayOpe['bn']="NOT LIKE";
            $arrayOpe['in']="IN";
            $arrayOpe['ni']="NOT IN";
            $arrayOpe['ew']="LIKE";
            $arrayOpe['en']="NOT LIKE";
            $arrayOpe['cn']="LIKE";
            $arrayOpe['nc']="NOT LIKE";
            return $arrayOpe[$oper];
        }

        /**
	 * obtiene el sql de la condicion de consulta para el grid dependiendo del operador de busqueda
	 *
	 * @access 	public
	 * @param 	string $field
         * @param 	string $oper
	 ...
	 * @return 	string
	 */
        public static function toSqlParamSearchGrid($field,$oper,$val){
            $ope= self::getOperatorLogicGrid($oper);
            
            //aplicamos los filtros para los campos de la busqueda

            filter::load('extraspaces','striptags','addslashes','stripspace','alnum');
            $filtro=new Filter();
            $filtro->addFilter(new ExtraspacesFilter());
            $filtro->addFilter(new StriptagsFilter());
            //$filtro->addFilter(new StripSpaceFilter());
            $filtro->addFilter(new StripSlashesFilter());
           // $filtro->addFilter(new AlnumFilter());
            $val=$filtro->applyFilter($val);
           
            if(strpos($ope,"LIKE")!==false){
                    if($oper=='bw' || $oper=='bn'){
                        $val="'".addslashes($val)."%'";
        ...
        /**
	 * obtiene el sql de la condicion de consulta para el grid en busqueda avanzada
	 *
	 * @access 	public
	 * @param 	object $filter
	 ...
	 */
        ...
        public static function constructWhereFilterGrid($filter){
                 $qwery='';
                 if ($filter) {

                       $jsona = json_decode($filter,true);
                       if(is_array($jsona)){
                            $gopr = $jsona['groupOp'];
                            $rules = $jsona['rules'];
                            $i =0;
                            foreach($rules as $key=>$val) {
                                $field = $val['field'];
                               /* if(key_exists($field,$arrayReplace) && is_array($arrayReplace))
                                {
                                    if(@$arrayReplace[$field]['op']!='')
                                        $op=$arrayReplace[$field]['op'];
                                    if(@$arrayReplace[$field]['data']!='')
                                        $v=$arrayReplace[$field]['data'];
                                }else{*/
                                    $op = $val['op'];
                                    $v = $val['data'];
                                //}
                                if($v && $op) {
                                     $i++;
                                     //  a sql
                                     $v=self::toSqlParamSearchGrid($field, $op, $v);
                                     if ($i == 1) $qwery = " AND ";
                                         else $qwery .= " " .$gopr." ";
                                        $qwery .= $v;
                                 }

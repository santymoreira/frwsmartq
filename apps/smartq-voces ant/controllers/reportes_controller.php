...
class ReportesController extends ApplicationController {
    ...
    public function turnosConTicketAction(){
        $this->setResponse('ajax');
        $ip_server = $_SERVER['SERVER_ADDR'];  
        $this->url_iframe = "http://$ip_server/smartq_reportes/ReportesConTicket/ReportesConTicket.php";
        $this->render('sucursal/reporte_turnos');      //carga la vista
    }
    
    public function turnosSinTicketAction(){
        $this->setResponse('ajax');
        $ip_server = $_SERVER['SERVER_ADDR'];  
        $this->url_iframe = "http://$ip_server/smartq_reportes/ReportesSinTicket/ReportesSinTicket.php";
        $this->render('sucursal/reporte_turnos');      //carga la vista
    }
...
//            $servicios= new Servicio();
//            $buscaServicios= $servicios->find("conditions: id IN ($lista_servicios_aux)");
//            foreach ($buscaServicios as $result) {
//                $arrayServicios[$result->getId()]=$result->getNombre();
//            }
//        }
//        print_r($this->arrayServicios);
//        die();
        //Inicio cÃ¯Â¿Â½lculo de dias entre fechas
...
//        $html.="</div>";
    ...
    public function GraficaBarras($titulo,$nombres,$total) {
        $strXML = "";
        $strXML = "<chart caption = 'Grafico 1: $titulo' bgColor='#CDDEE5' baseFontSize='12' showValues='1' xAxisName='Modulos' formatNumberScale='0' thousandseparator='.' labelDisplay='ROTATE'>";
...
//            die();
            foreach($listaTotales as $key=>$val) {
                $strXML .= "<set label = '".$listanombres[$key+1]. "' value ='".$listaTotales[$key]."'/>";
                $x+=10;
                $y=480;
...
//            die();
        ...
        if($tipo=="barras") {
            ...
            if ($resultado!=0)
                $graficaCalidad=$this->Graficadorpie("Calidad",$lista_nombres_calidad,$lista_datos);

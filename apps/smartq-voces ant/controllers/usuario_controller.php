...
class UsuarioController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $columnsGrid[] = array('name' => 'Role(s)', 'index' => 'grupo', 'width' => 280);
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        //-- ver los grupos seleccionados
        $obj_grupo = new Grupo();
        $grupo = $obj_grupo->find();
        $array_grupos_seleccionados = array();
        foreach ($grupo as $result) {
            $array_grupos_seleccionados[] = $this->getPostParam("chk_grupo_{$result->getId()}");
        }
        $array_grupos_seleccionados = array_filter($array_grupos_seleccionados);
        
        //print_r($array_grupos_seleccionados); die();
        ...
        if ($usuario->save() == false) {
            ...
            if ($isEdit == true) {
                //INICIO ELIMINAR grupousuario
                $grupo_usuario = new Grupousuario();
                $grupo_usuario->deleteAll($whereCondition = "usuario_id= $id");
                //INICIO GUARDAR GRUPOUSUARIO
                foreach ($array_grupos_seleccionados as $grupo_id) {
                    //echo $grupo_id;
                    $grupo_usuario = new Grupousuario();
                    $grupo_usuario->setUsuarioId($id);
                    $grupo_usuario->setGrupoid($grupo_id);
                    $grupo_usuario->save();
                }
                //die();
                //FIN GUARDAR GRUPOUSUARIO
        ...
        for ($i = $inicio; $i < $limite; $i++) {
            ...
            if ($Usuario->getUsername() != "superadmin") {
                ...
                foreach ($query->getResultSet() as $result) {
                    $array_roles[] = $result->getNombreLargo();
                }
                $grupo_acceso = implode(', ', $array_roles);

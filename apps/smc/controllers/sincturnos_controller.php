    ...
    */
    public function sincronizacionAction() {
        ...
        //foreach ($chk_sucursales as $key) {
            ...
            $db1->query("CREATE TEMPORARY TABLE tmp_turnos(id INT NOT NULL AUTO_INCREMENT , id_referencia INT , sucursal_id INT ,numeroModulo INT,moduloId INT,servicioId INT, usuario VARCHAR(50) , nombre_servicio  VARCHAR(50) ,  numero_turno  VARCHAR(5) ,  fecha_emision  DATE ,  hora_emision  TIME ,  fecha_inicio_atencion  DATE ,  hora_inicio_atencion  TIME ,  fecha_fin_atencion  DATE ,  hora_fin_atencion  TIME , atendido INT,rechazado INT, duracion  TIME ,  calificacion  VARCHAR(20) , PRIMARY KEY ( id )) ;");
            ...
            $db->query("SELECT t.id AS id_referencia, alias_empresa,t.numero as turnoNumero,t.servicio_id as servicioId,t.caja_id as cajaId, u.nombres AS usuario, s.nombre AS nombre_servicio,
                numero AS numero_turno, fecha_emision, hora_emision, fecha_inicio_atencion, hora_inicio_atencion,
                fecha_fin_atencion, hora_fin_atencion, t.atendido,t.rechazado,duracion, calificacion
                FROM servicio s, turnos t, caja c, usuario u, usercaja uc, empresa e
                WHERE u.id=uc.usuario_id AND c.id=uc.caja_id AND s.id=t.servicio_id AND c.id=t.caja_id
            ...
            if (!empty($db)) {
                while($row = $db->fetchArray()) {
                    ...
                    $db1->query("INSERT INTO tmp_turnos (id_referencia, sucursal_id,numeroModulo,moduloId,servicioId, usuario,nombre_servicio,numero_turno,fecha_emision,hora_emision,fecha_inicio_atencion,hora_inicio_atencion,fecha_fin_atencion,hora_fin_atencion,atendido,rechazado,duracion,calificacion)
                        VALUES ('$idReferencia','$sucursal_id','$numeroModulo','$CajaId','$ServicioId','$usuario','$nombreServicio',$numeroTurno,'$fechaEmision','$horaEmision','$fechaInicioAtencion','$horaInicioAtencion','$fechaFinAtencion','$horaFinAtencion','$atendido','$rechazado','$duracion','$calificacion')");
                }
            }
            ...
            $result=$db1->query("SELECT id_referencia, sucursal_id,numeroModulo,moduloId,servicioId,usuario,nombre_servicio,numero_turno,fecha_emision,hora_emision,fecha_inicio_atencion,hora_inicio_atencion,fecha_fin_atencion,hora_fin_atencion,atendido,rechazado,duracion,calificacion
            ...
            if (!empty($db1)) {
                ...
                while($row = $db1->fetchArray($result)) {
                    ...
                    $numero=$row['numeroModulo'];
                    $IdModulo=$row['moduloId'];
                    $IdServicio=$row['servicioId'];
                    ...
                    $fechaFinAtencion = $row['fecha_fin_atencion'];
                    $horaFinAtencion = $row['hora_fin_atencion'];
                    $tAtendido=$row['atendido'];
                    $tRechazado=$row['rechazado'];
                    ...
                    $sincturnos->setNumeroModulo($numero);
                    $sincturnos->setModuloIdSucursal($IdModulo);
                    $sincturnos->setServicioIdSucursal($IdServicio);
                    ...
                    $sincturnos->setAtendido($tAtendido);
                    $sincturnos->setRechazado($tRechazado);

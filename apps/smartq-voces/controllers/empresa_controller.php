...
class EmpresaController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $columnsGrid[] = array('name' => 'Ruc', 'index' => 'ruc');
        $columnsGrid[] = array('name' => 'Razón Social', 'index' => 'razonsocial');
        $columnsGrid[] = array('name' => 'Matriz', 'index' => 'matriz', 'width' => '38px');
        ...
        $columnsGrid[] = array('name' => 'Calif Matriz Pantalla', 'index' => 'calif_matriz_pantalla', 'width' => '124px');
        $columnsGrid[] = array('name' => 'Disp Simple', 'index' => 'dispensador_simple', 'width' => '70px');
        $columnsGrid[] = array('name' => 'Disp Touch', 'index' => 'dispensador_touch', 'width' => '70px');
        $columnsGrid[] = array('name' => 'Disp Botonera', 'index' => 'dispensador_botonera', 'width' => '90px');
        $columnsGrid[] = array('name' => 'Disp Touch Pequeña', 'index' => 'dispensador_touch_pequenia', 'width' => '125px');
        $columnsGrid[] = array('name' => 'Carpeta', 'index' => 'carpeta', 'width' => '80px');
        $columnsGrid[] = array('name' => 'Selección Operador', 'index' => 'seleccion_operador', 'width' => '120px');
        $columnsGrid[] = array('name' => 'Servidor Node', 'index' => 'servidor_node','width'=>120);
        $columnsGrid[] = array('name' => 'Conf. Sucursales', 'index' => 'configura_sucursales','width'=>120);
        $columnsGrid[] = array('name' => 'Areas', 'index' => 'areas','width'=>120);
        $columnsGrid[] = array('name' => 'Pantalla', 'index' => 'pantalla', 'width'=>120);
    ...
    public function editarAction($id = null) {
        ...
        if ($empresa) {
            ...
            Tag::displayTo('ruc', $empresa->getRuc());
            ...
            Tag::displayTo('configura_sucursales', $empresa->getConfiguraSucursales());

            Tag::displayTo('pantalla', $empresa->getPantalla());

            $servidor_node = explode(":", $empresa->getServidorNode());
            if (count($servidor_node)>1){
                Tag::displayTo('ip_servidor_node', $servidor_node[0]);
                Tag::displayTo('puerto_servidor_node', $servidor_node[1]);

                //Tag::displayTo('areas', base64_decode($empresa->getAreas()));
                Tag::displayTo('areas', Encrypter::decrypt($empresa->getAreas()));
            }
        } else {
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        $ruc = $this->getPostParam("ruc", "striptags", "extraspaces");
        ...
        if ($moduloOperadores == 'on')
            $moduloOperadores = 1;
        $moduloCajas = $this->getPostParam("modulo_cajas");
        if ($moduloCajas == 'on')
            $moduloCajas = 1;
        $calif4botonesTeclado = $this->getPostParam("calif_4botones_teclado");
        if ($calif4botonesTeclado == 'on')
            $calif4botonesTeclado = 1;
        $calif4botonesPantalla = $this->getPostParam("calif_4botones_pantalla");
        if ($calif4botonesPantalla == 'on')
            $calif4botonesPantalla = 1;
        $califMatrizPantalla = $this->getPostParam("calif_matriz_pantalla");
        if ($califMatrizPantalla == 'on')
            $califMatrizPantalla = 1;
        $dispensadorSimple = $this->getPostParam("dispensador_simple");
        if ($dispensadorSimple == 'on')
            $dispensadorSimple = 1;
        $dispensadorTouch = $this->getPostParam("dispensador_touch");
        if ($dispensadorTouch == 'on')
            $dispensadorTouch = 1;
        $dispensadorBotonera = $this->getPostParam("dispensador_botonera");
        if ($dispensadorBotonera == 'on')
            $dispensadorBotonera = 1;
        $dispensadorTouchPequenia = $this->getPostParam("dispensador_touch_pequenia");
        if ($dispensadorTouchPequenia == 'on')
            $dispensadorTouchPequenia = 1;
        $seleccionOperador = $this->getPostParam("seleccion_operador");
        if ($seleccionOperador == 'on')
            $seleccionOperador = 1;
        $ipServidorNode = $this->getPostParam("ip_servidor_node", "striptags", "extraspaces");
        $puertoServidorNode = $this->getPostParam("puerto_servidor_node", "striptags", "extraspaces");
        $servidorNode = "$ipServidorNode:$puertoServidorNode";
        $configuraSucursales = $this->getPostParam("configura_sucursales");
        if ($configuraSucursales == 'on')
            $configuraSucursales = 1;
        ...
        $pantalla = $this->getPostParam("pantalla", "striptags", "extraspaces");
        if ($pantalla == 'on')
            $pantalla = 1;
        ...
        /* MENU
         * 48 = Configuracion/Sucursales/Datos sucursales
         * 46 = Configuracion/Sucursales/Sincronizar
         * 55 = Administracion/Sucursales/Reportes/Turnos con ticket
         * 57 = Administracion/Sucursales/Reportes/Turnos sin ticket
         */
        if ($matriz == 1) {
        ...
        //FIN DAR PERMISOS PARA MATRIZ PARA SUCURSALES
        //INICIO DAR PERMISOS PARA MODULO ATENCION CON/SIN TICKET
        /* MENU
         * 30 = Administracion/Local/Reportes/Turnos con ticket
         * 33 = Administracion/Local/Reportes/Turnos sin ticket
         */ 
        $menu = new Menu();
        if ($moduloOperadores == 1) {
            $menu->updateAll("estado=1", "id IN (30)");
        } else {
            $menu->updateAll("estado=0", "id IN (30)");
        }
        if ($moduloCajas == 1) {
            $menu->updateAll("estado=1", "id IN (33)");
        } else {
            $menu->updateAll("estado=0", "id IN (33)");
        }
        //FIN DAR PERMISOS PARA MODULO ATENCION CON/SIN TICKET
        
        if ($configuraSucursales!=1){
            $menu = new Menu();
            /*
            Configuracion/Sucursales = 50
            Configuracion/Sucursales/Datos Sucursales = 48
            Administracion/Sucursales
             */
            $menu->updateAll("estado=0", "id IN (50,48,53)");
        } else {
            $menu->updateAll("estado=1", "id IN (50,48,53)");
        }
        ...
        $empresa->setServidorNode($servidorNode);
        //
        //$empresa->setAreas(base64_encode($areas));
        $empresa->setAreas(Encrypter::encrypt($areas));
        $empresa->setPantalla($pantalla);
        //
        $empresa->setConfiguraSucursales($configuraSucursales);
    ...
    public function obtenerDatosGridAction() {
        ...
        for ($i = $inicio; $i < $limite; $i++) {
            ...
            if(    is_numeric(Encrypter::decrypt($Empresa->getAreas()))  ){
                $areasE=Encrypter::decrypt($Empresa->getAreas());
            }else{
                $areasE="";
            }

            $jqgrid->rows[] = array('id' => $Empresa->getId(), 'cell' => array($Empresa->getId(), $Empresa->getNombrecomercial(), $Empresa->getRuc(), $Empresa->getRazonsocial(), $Empresa->getMatriz(), $Empresa->getDireccion(), $Empresa->getModuloOperadores(), $Empresa->getModuloCajas(), $Empresa->getCalif4botonesTeclado(), $Empresa->getCalif4botonesPantalla(), $Empresa->getCalifMatrizPantalla(), $Empresa->getDispensadorSimple(), $Empresa->getDispensadorTouch(), $Empresa->getDispensadorBotonera(), $Empresa->getDispensadorTouchPequenia(), $Empresa->getCarpeta(), $Empresa->getSeleccionOperador(), $Empresa->getServidorNode(), $Empresa->getConfiguraSucursales(), $areasE, $Empresa->getPantalla()));
        }

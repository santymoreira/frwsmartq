...
class ServicioController extends ApplicationController {
    ...
    public function indexAction() {
        ...
        $columnsGrid[] = array('name' => 'Atención Preferencial', 'index' => 'letra_alias', 'width' => 120);
        $columnsGrid[] = array('name' => 'Letra A. Preferencial', 'index' => 'letra_alias', 'width' => 120);
    ...
    public function guardarAction($isEdit = false, $id = null) {
        ...
        $letra = strtoupper($this->getPostParam("letra", "striptags", "extraspaces"));
        ...
        $letraAlias = strtoupper($this->getPostParam("letra_alias", "striptags", "extraspaces"));
        ...
        if ($letraAlias == '') {
            ...
            $wh2 = "(letra IN('$letra') OR letra_alias IN ('$letra'))";
        } else {
            $servicio->setLetraAlias($letraAlias);
            $wh2 = "(letra IN('$letra','$letraAlias') OR letra_alias IN ('$letra','$letraAlias'))";
        }
        //__________________
        ...
        //} else {
            ...
            if ($existe_letra >= 1) {
                while ($row = $db->fetchArray($result)) {
                    ...
                    $letra_alias_asignada = $row['letra'];
                }
                if ($letra_asignada == $letra & $letra_alias_asignada == $letraAlias)
                    Flash::error("La(s) letra(s) $letra, $letraAlias está asignada a un servicio.");
                if ($letra_asignada == $letra)
                    Flash::error("La letra $letra está asignada a un servicio.");
                else
